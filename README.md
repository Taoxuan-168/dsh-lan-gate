...
- **设备码**：每台新设备访问时自动分配一个 **6 位大写设备码**（去易混淆字符，显示在设备门禁页），管理员按设备码批准/拒绝，不用记 IP；设备码随批准记录持久化，重启不丢
- **命令行管理**：`node lan-gate.mjs approve <设备码>` 等命令可直接批准/拒绝/撤销设备（走本机管理接口，改完立即生效，无需重启）
- **每台设备指定访问方式**：
  - 自动 —— 按屏幕宽度适配
...
2. **验证插件已加载**（二选一）：
   - 浏览器打开 `http://127.0.0.1:3080/lan-gate/status`，看到类似
     `{"state":"stopped","port":3088,...}` 就成功了（内网访问**默认不自动开启**，需手动启动，见下方「使用」）；
...
| 内网穿透后所有设备都显示为 `127.0.0.1`、无法拦截新设备、频繁 429 | 穿透工具在本机回环转发流量，源 IP 是 `127.0.0.1`。请使用**会透传真实 IP 的穿透方式**（frp http 类型、ngrok、Cloudflare Tunnel 等都会带 `X-Forwarded-For` 头）；插件会自动按 XFF 里的真实 IP 走门禁与限流。纯 TCP 转发（无 XFF）无法区分来源，请改用 http 类型或开启 proxy protocol |
| 用反向代理/隧道转发到本插件时全部被拒 | 未批准的真实来源 IP 会走门禁（拒绝页/等待批准页），这是预期行为：先在面板或命令行批准该设备即可。生产环境建议在**前置代理**上配置登录认证，并把 `LISTEN_HOST` 改为 `127.0.0.1` 只让本机代理接入 |

## 使用

> ⚠️ **内网访问默认不自动开启**：每次重启 DSH 后，内网访问处于「已停止」状态。请先手动开启（二选一）：
> - 命令行：`node lan-gate.mjs start`
> - 面板：DSH 设置 → 内网访问 → 点「启动 / 重启」按钮

1. 打开 DSH 设置（左下角齿轮）→ **内网访问**：面板显示运行状态与全部访问地址（点击可复制）
2. 把地址发给手机（微信/QQ 发给自己即可），手机浏览器打开
3. 手机显示「等待本机批准」并给出**设备码**（如 `K7M2PX`），本机在面板的「按设备码批准」输入框输入该码（或直接在待批准列表点允许），手机自动进入 DSH
4. 随时在面板中切换每台设备的访问方式、撤销、或停止服务

### 命令行管理（无需重启）

> 💡 **短命令 `dsh-lan`**：可创建 `/usr/local/bin/dsh-lan` 包装脚本（内容为 `exec node "${DSH_HOME:-$HOME/.dsh}/lan-gate/lan-gate.mjs" "$@"` 并 `chmod +x`），之后用 `dsh-lan status`、`dsh-lan approve <设备码>` 等代替完整路径。本机已安装，直接可用。

```bash
node lan-gate.mjs status                                # 查看状态与待批准设备码
...
node lan-gate.mjs revoke <设备码|IP>                    # 撤销已批准设备
node lan-gate.mjs clear <设备码|IP>                     # 清除设备记录，使其完全不可见（下次访问视为新设备）
node lan-gate.mjs stop                                  # 停止内网访问（关闭 3088 端口）
node lan-gate.mjs start                                 # 重新开启内网访问
```

命令通过本机 `127.0.0.1:3080` 的管理接口操作运行中的插件，改完立即生效。
